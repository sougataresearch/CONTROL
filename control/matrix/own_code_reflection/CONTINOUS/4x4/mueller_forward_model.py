...
Verbatim duplicate of DISCRETE/4x4/mueller_forward_model.py -- the physics
is identical whether the QWP steps through discrete angles or spins
continuously; only how the (angle, intensity) pairs are gathered differs
(see image_loader.py in this folder). PSG_Polarizer/PSA_Analyzer are held
fixed for the whole run (their angles come from
Config/experiment_config.json's "fixed_angles"), while PSG_QWP/PSA_QWP
rotate. Both the polarizer and the retarder Mueller matrices are built
directly from the physical rotation sandwich
