"""Validate the reflection-mode 3x3 solver against samples with a
computable theoretical Mueller matrix (bare substrate, or substrate + one
thin film -- see reflection_theory.py/theoretical_mueller.py, both reduced
to their S0,S1,S2 sub-block for 3x3 mode), using the exact same
...
You'll then be prompted for the polarizer extinction ratio (shared with
main.py's .last_calibration.json in this folder), applied to every sample
so they're all reconstructed on an equal footing -- and, per sample, for
that sample's physical parameters as described above.

If main.py already reconstructed a given sample with this exact
extinction_ratio (saved under its default Results/<date-relative-path>
location), that cached reconstruction is reused instead of redoing it from
raw images.
"""
...
def _load_cached_reconstruction(sample_dir: Path, extinction_ratio: float):
    ...
    cache_dir = (RESULT_ROOT / "reflection" / "3x3" / "reconstructions"
...
def main() -> None:
    ...
    base_dir = RESULT_ROOT / "reflection" / "3x3" / "validation_against_theory"
