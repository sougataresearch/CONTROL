"""3x3 mode only sees the S0,S1,S2 (linear-polarization) sub-block of a
sample's true Mueller matrix -- same limitation as the transmission-mode
3x3 tools. This module computes the full 4x4 theoretical reflection matrix
internally and returns its top-left 3x3 sub-block, ignoring the S3
(circular polarization) row/column entirely, consistent with that
limitation.
...
def bare_substrate_mueller(substrate_n: float, substrate_k: float,
    ...
    """Theoretical 3x3 (S0,S1,S2 sub-block) reflection Mueller matrix for a
    bare substrate (single interface, air -> substrate), normalized so
    M[0,0]=1."""
...
def film_on_substrate_mueller(substrate_n: float, substrate_k: float,
    ...
    """Theoretical 3x3 (S0,S1,S2 sub-block) reflection Mueller matrix for a
    substrate with one thin film on top (e.g. SiO2 on Si), normalized so
    M[0,0]=1."""
