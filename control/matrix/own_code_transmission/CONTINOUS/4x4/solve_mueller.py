"""Generalized least-squares 4x4 Mueller matrix reconstruction from N frames
of a continuous PSG_QWP/PSA_QWP rotation.

Verbatim in approach to DISCRETE/4x4/solve_mueller.py: takes however many
(PSG_QWP angle, PSA_QWP angle, intensity) samples a run actually has -- 60,
360, 720, whatever -- with no assumption about a discrete angle grid. One
system-matrix row per frame, built from the same rotation-sandwich physics
in mueller_forward_model.py (fixed polarizer + rotating QWP); solved per
pixel in a single vectorized least-squares fit. This is why a continuous
sweep needs no separate Fourier/harmonic-analysis math: more, non-grid
samples over one revolution just add more (well-distributed) rows to the
same linear system a handful of discrete-angle images would build.

Works for any sample -- the sample's identity never enters this code, only
the known fixed/rotating PSG/PSA angles and the measured intensities.
"""
...
def _build_system_matrix(
    ...
    """One row per frame: kron(analyzer_vector, generator_stokes)."""
