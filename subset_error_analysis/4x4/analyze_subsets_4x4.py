"""Standalone tool: for every over-determined 4x4-Mueller-matrix capture found
under a data root (more than the minimum 16 images = 4 PSG_QWP angles x 4
PSA_QWP angles), compare the reconstruction error of the full over-determined
fit against every possible 16-image (4-angle x 4-angle) subset drawn from the
same data, ranked against the known theoretical matrix.

Answers, per sample and overall:
  1. Does using every captured image actually beat every 16-image subset?
  2. Which specific 4-angle QWP combo gives the lowest deviation from
     theory, and is that consistent across samples?
...
# scans every subfolder of DATA_ROOT for 4x4-mode runs.
# ---------------------------------------------------------------------------
DATA_ROOT = r"C:\COMPARE_CASES\Data"

# How many QWP angles per side to draw into each candidate subset (4 ->
# 16-image combinations, matching the minimum acquisition for a 4x4 matrix).
SUBSET_SIZE = 4

# A run is only usable for this analysis if it has at least this many unique
# QWP angles per side (need >= SUBSET_SIZE to form any subset, and >
# SUBSET_SIZE for the comparison to be meaningful against an over-determined
# baseline).
...
# rotation sandwich M(theta) = R(-theta) @ M(0) @ R(theta) as 3x3, just with
# the retarder included and full 4x4 matrices (no linear-polarization-only
# sub-block reduction).
# ---------------------------------------------------------------------------
...
def discover_runs(data_root: str) -> list[Path]:
    """Recursively find every folder under data_root that is a 4x4-mode run
    (has Config/experiment_config.json with mode == '4x4')."""
    root = Path(data_root)
    runs = []
...
def analyze_run(run_dir: Path) -> dict | None:
    psg_qwp_angles, psa_qwp_angles, images, fixed_angles, sample_name = load_run(run_dir)
    ...
    if len(unique_angles) < MIN_UNIQUE_ANGLES:
        print(f"  Skipping {sample_name} ({run_dir}): only {len(unique_angles)} "
              f"unique QWP angle(s) present, need >= {MIN_UNIQUE_ANGLES} for this analysis.")
        return None
...
def main() -> None:
    print(f"Scanning for 4x4-mode runs under: {DATA_ROOT}\n")
    runs = discover_runs(DATA_ROOT)

    if not runs:
        print("No 4x4-mode runs found. Check DATA_ROOT at the top of this script.")
        return
    ...
    if answer not in ("y", "yes"):
        print("Aborting. Adjust DATA_ROOT at the top of analyze_subsets_4x4.py and rerun.")
        return
    ...
    if not results:
        print("\nNo runs were suitable for subset analysis (need an over-determined "
              f"grid with >= {MIN_UNIQUE_ANGLES} unique QWP angles per side).")
        return
